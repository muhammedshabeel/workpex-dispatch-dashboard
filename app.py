...
st.set_page_config(
    page_title="Workpex Dispatch Dashboard",
    page_icon="📦",
    layout="wide",
)
...
uploaded = st.file_uploader(
    ...
    type=["xlsx", "xls"],
    help="Expected format: Workpex order export",
)
...
try:
    ...
    ksa_result = build_ksa_exports(source_df)
except (DispatchError, ValueError, FileNotFoundError) as exc:
    st.error(str(exc))
    st.stop()
...
m6.metric(
    "KSA",
    f"{ksa_result.naqel_orders + ksa_result.jnt_orders:,}",
    help=f"Naqel: {ksa_result.naqel_orders:,} | JNT: {ksa_result.jnt_orders:,}",
)
...
with buttons[0]:
    st.download_button(
        "Naqel KSA",
        data=ksa_result.naqel_bytes,
        file_name="Naqel_KSA.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        width="stretch",
        disabled=ksa_result.naqel_orders == 0,
    )
    st.download_button(
        "JNT KSA",
        data=ksa_result.jnt_bytes,
        file_name="JNT_KSA.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        width="stretch",
        disabled=ksa_result.jnt_orders == 0,
    )
    st.caption(
        f"Naqel: {ksa_result.naqel_orders:,} orders | JNT: {ksa_result.jnt_orders:,} orders"
    )
...
preview_options = {
    ...
    "KSA - Naqel": ksa_result.naqel_df,
    "KSA - JNT": ksa_result.jnt_df,
}
...
- **Bahrain Dispatch:** Bahrain portal format with case-sensitive constants, city/zone mapping, Product 2 handling, Agent Name, Source, and Payment Method.
- **Qatar Dispatch:** Qatar portal format with case-sensitive constants, city/zone mapping, Product 2 handling, Agent Name, Source, and Payment Method.
- **KSA Dispatch:** KSA orders are split into Naqel and JNT using the approved Naqel product list and courier city coverage.
- **KSA Product 2:** Same product merges quantity; a different second product is exported as an additional product line.
